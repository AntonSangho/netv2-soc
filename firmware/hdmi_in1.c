...
int hdmi_in1_phase_startup(freq)
{
	int ret;
	int attempts;
			...
			if(hdmi_in1_locked) {
				if(hdmi_in1_clocking_locked_filtered()) {
					if(elapsed(&last_event, SYSTEM_CLOCK_FREQUENCY/2)) {
					  ...
					  hdmi_in1_data1_wer_update_write(1);
					  hdmi_in1_data2_wer_update_write(1);
					  if( hdmi_in1_data0_wer_value_read() + hdmi_in1_data1_wer_value_read() +
					      hdmi_in1_data2_wer_value_read() == 0 ) {
					    got_zero = 1;
					  } else if( hdmi_in1_data0_wer_value_read() + hdmi_in1_data1_wer_value_read() +
						     hdmi_in1_data2_wer_value_read() > 10 ) {
					    got_zero = 0;
					  }
